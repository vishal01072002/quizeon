    ...
    <>
      {
        ...
        <div className='bg-white p-5 relative'>
          {/* top div */}
          <div className='flex relative gap-10 justify-end w-[95%] lg:w-10/12 mx-auto mb-8 mt-5'>
            <div onClick={() => navigate(`/analytic/${quizId}`)} className='absolute cursor-pointer left-5 px-3 py-2 bg-purple-500 rounded-sm text-white '>Back</div>
            <div className=' p-2 flex flex-col items-end text-start'>
          ...
          <div className='flex items-center flex-col gap-4 p-4'>
            <div className='flex w-[95%] lg:w-10/12 mx-auto mb-5 gap-3 justify-between font-semibold text-lg text-gray-600'>
              ...
              <p className='flex-1'>CompletedTime</p>
            ...
            </div>
            {
              ...
              leaderBoardData?.quiz?.scoreList.map((score,indx) => (
                <div className='flex w-[95%] lg:w-10/12 gap-3 text-lg justify-between items-start border-b-2 pb-2 border-gray-300 shadow-sm rounded-md hover:scale-105 duration-200 cursor-pointer' key={score?._id}
                onClick={() => showScore({...score, rank: `${(indx+1) + (leaderBoardData?.scorePerPage * (pageNo-1))}${(pageNo === '1' && indx === 0) ? "st" : (pageNo === '1' && indx === 1) ? "nd" : (pageNo === '1' && indx === 2) ? "rd" : "th"}`})}>
                  <p className='w-10 text-xl text-purple-800 font-bold text-center'>{(indx+1) + (leaderBoardData?.scorePerPage * (pageNo-1))}{(pageNo === '1' && indx === 0) ? "st" : (pageNo === '1' && indx === 1) ? "nd" : (pageNo === '1' && indx === 2) ? "rd" : "th"}</p>
                  ...
                  <p className='flex-1 grow text-center'>{score?.completedTime?.split(",").at(0)}:{score?.completedTime.split(",").at(1)} min</p>
                  <div className='flex-1 hidden md:flex md:min-w-[220px] justify-center'>
                    <p className='w-16 font-semibold text-start text-green-500'>{score?.correct}</p>
                    <p className='w-10 font-semibold text-start text-red-500'>{score?.wrong}</p>
                    <p className='w-14 font-semibold text-center text-blue-500'>{score?.unAttempted}</p>
                  </div>
                </div>
              ))    
            }
